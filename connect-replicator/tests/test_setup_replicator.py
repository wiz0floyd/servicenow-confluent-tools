...
@pytest.fixture
def tmp():
    """Provide a temporary directory path; cleanup handled by TemporaryDirectory."""
    with tempfile.TemporaryDirectory() as d:
        yield d


# =========================================================================
# build_cc_to_sn_config / build_sn_to_cc_configs / build_topic_regex
# =========================================================================


def test_cc_to_sn_has_producer_override_keys():
    cfg = sr.build_cc_to_sn_config(
        "repl", "host", 4000, 4,
        FAKE_CA, FAKE_CERT, FAKE_KEY, None,
        "grp", ".*", "${topic}",
    )
    assert "producer.override.bootstrap.servers" in cfg
    assert "producer.override.security.protocol" in cfg
    # Must NOT have src.kafka.* keys
    src_keys = [k for k in cfg if k.startswith("src.kafka.")]
    assert src_keys == []


def test_sn_to_cc_has_src_kafka_keys():
    ...
    cfg = configs[0]
    assert "src.kafka.bootstrap.servers" in cfg
    assert "src.kafka.security.protocol" in cfg
    # Must NOT have producer.override.* keys
    po_keys = [k for k in cfg if k.startswith("producer.override.")]
    assert po_keys == []


def test_cc_to_sn_key_password_present():
    cfg = sr.build_cc_to_sn_config(
        "repl", "host", 4000, 4,
        FAKE_CA, FAKE_CERT, FAKE_KEY, "s3cret",
        "grp", ".*", "${topic}",
    )
    assert cfg["producer.override.ssl.key.password"] == "s3cret"


def test_cc_to_sn_key_password_absent():
    cfg = sr.build_cc_to_sn_config(
        "repl", "host", 4000, 4,
        FAKE_CA, FAKE_CERT, FAKE_KEY, None,
        "grp", ".*", "${topic}",
    )
    assert "producer.override.ssl.key.password" not in cfg


def test_sn_to_cc_key_password_present():
    configs = sr.build_sn_to_cc_configs(
        "repl", "host", [4100], 4,
        FAKE_CA, FAKE_CERT, FAKE_KEY, "s3cret",
        "grp", ".*", "${topic}",
    )
    assert configs[0]["src.kafka.ssl.key.password"] == "s3cret"


def test_sn_to_cc_key_password_absent():
    configs = sr.build_sn_to_cc_configs(
        "repl", "host", [4100], 4,
        FAKE_CA, FAKE_CERT, FAKE_KEY, None,
        "grp", ".*", "${topic}",
    )
    assert "src.kafka.ssl.key.password" not in configs[0]


def test_topic_regex_all():
    regex = sr.build_topic_regex(None, use_all=True)
    assert "__" in regex  # negative lookahead for __
    # Verify it is the catch-all pattern
    assert regex == "(?!__)(?!_confluent).+"


def test_topic_regex_selected():
    regex = sr.build_topic_regex(["t1", "t2"], use_all=False)
    assert regex == "^(t1|t2)$"


def test_topic_regex_dot_escaped():
    regex = sr.build_topic_regex(["my.topic"], use_all=False)
    assert "my\\.topic" in regex


def test_cc_to_sn_connector_name_plain():
    cfg = sr.build_cc_to_sn_config(
        "my-repl", "host", 4000, 4,
        FAKE_CA, FAKE_CERT, FAKE_KEY, None,
        "grp", ".*", "${topic}",
    )
    assert cfg["name"] == "my-repl"


def test_sn_to_cc_connector_name_appends_port():
    configs = sr.build_sn_to_cc_configs(
        "my-repl", "host", [4100, 4200], 4,
        FAKE_CA, FAKE_CERT, FAKE_KEY, None,
        "grp", ".*", "${topic}",
    )
    assert configs[0]["name"] == "my-repl-4100"
    assert configs[1]["name"] == "my-repl-4200"
...
def test_direction_defaults_cc_to_sn(tmp):
    path = _write_conf(tmp, MINIMAL_CONF)
...
def test_connect_url_default(tmp):
    path = _write_conf(tmp, MINIMAL_CONF)
    cfg = sr.load_config(path)
    # connect_url defaults in main() via .get
    assert "connect_url" not in cfg


def test_missing_required_key_exits(tmp):
    bad_conf = "[confluent]\nenvironment_id = env-abc\n"
    path = _write_conf(tmp, bad_conf)
    with pytest.raises(SystemExit) as exc:
        sr.load_config(path)
    assert exc.value.code == 1


def test_missing_file_exits(tmp):
    with pytest.raises(SystemExit):
        sr.load_config(os.path.join(tmp, "nonexistent.conf"))


def test_topics_override(tmp):
    conf = MINIMAL_CONF + "topics = alpha,beta\n"
    path = _write_conf(tmp, conf)
    cfg = sr.load_config(path)
    assert cfg.get("topics") == "alpha,beta"


def test_pem_dir_override(tmp):
    conf = MINIMAL_CONF.replace("pem_dir = /tmp/pems", "pem_dir = /other/pems")
    path = _write_conf(tmp, conf)
    cfg = sr.load_config(path)
    assert cfg["pem_dir"] == "/other/pems"


def test_source_clusters_parses_csv(tmp):
    conf = MINIMAL_CONF + "source_clusters = 5100,5200,5300\n"
    path = _write_conf(tmp, conf)
    cfg = sr.load_config(path)
    assert cfg["source_clusters"] == [5100, 5200, 5300]


def test_source_clusters_default(tmp):
    path = _write_conf(tmp, MINIMAL_CONF)
    cfg = sr.load_config(path)
    assert cfg["source_clusters"] == list(sr.SN_SOURCE_CLUSTERS)


# =========================================================================
# list_cc_topics / list_sn_topics
# =========================================================================
...
def _mock_urlopen_responses(states):
    """Return a list of mock responses yielding status JSON for each state."""
    responses = []
...
def _write_full_conf(tmp, pem_dir, extra=""):
...
def test_direction_sn_to_cc_all_produces_two_configs(mock_cli_check, mock_urlopen):
    ...
    assert output.count("--- Connector:") == 2
    mock_urlopen.assert_not_called()
